...
public class CourseEnrollmentRequestValidator extends BaseRequestValidator {
  ...
  public void validateLearningPathwayContent(String id, RequestContext requestContext) {
    logger.info(requestContext, "Validating Learning Pathway content for id: " + id);

    List<String> fields = Arrays.asList(JsonKey.STATUS, JsonKey.COURSECATEGORY, JsonKey.BATCHES, JsonKey.MILESTONES_V1, JsonKey.PRE_ENROLMENT_RESOURCES);

    Map<String, Object> contentResponse = ContentUtil.getContent(id, fields);
    Map<String, Object> contentData = (Map<String, Object>) contentResponse.get(JsonKey.CONTENT);

    if (MapUtils.isEmpty(contentData)) {
      throw new ProjectCommonException(
              ResponseCode.resourceNotFound.getErrorCode(),
    ...
    // Validate content status is Live
    String status = (String) contentData.get(JsonKey.STATUS);
    if (!JsonKey.LIVE.equalsIgnoreCase(status)) {
      throw new ProjectCommonException(
              ResponseCode.invalidParameterValue.getErrorCode(),
              "Content status must be Live. Current status: " + status,
              ResponseCode.CLIENT_ERROR.getResponseCode()
      );
    }

    // Validate course category
    String courseCategory = (String) contentData.get(JsonKey.COURSECATEGORY);
    if (!"Learning Pathway".equalsIgnoreCase(courseCategory)) {
      throw new ProjectCommonException(
              ResponseCode.invalidParameterValue.getErrorCode(),
              "Course category must be 'Learning Pathway'. Current category: " + courseCategory,
              ResponseCode.CLIENT_ERROR.getResponseCode()
      );
    }

    // Validate batches are available
    Object batchesObj = contentData.get(JsonKey.BATCHES);
    if (!(batchesObj instanceof List) || ((List<?>) batchesObj).isEmpty()) {
      throw new ProjectCommonException(
              ResponseCode.courseDoesNotHaveBatch.getErrorCode(),
              "No batches available for Learning Pathway",
              ResponseCode.CLIENT_ERROR.getResponseCode()
      );
    }

    // Validate milestones have assessment details
    Object milestonesObj = contentData.get(JsonKey.MILESTONES_V1);
    if (!(milestonesObj instanceof List) || ((List<?>) milestonesObj).isEmpty()) {
      throw new ProjectCommonException(
              ResponseCode.invalidParameterValue.getErrorCode(),
    ...
    List<Map<String, Object>> milestones = (List<Map<String, Object>>) milestonesObj;
    for (Map<String, Object> milestone : milestones) {
      Map<String, Object> assessmentDetailObj = (Map<String, Object>) milestone.get("assessmentDetail");

      if (MapUtils.isEmpty(assessmentDetailObj)) {
        throw new ProjectCommonException(
                ResponseCode.invalidParameterValue.getErrorCode(),
                "Assessment detail missing for milestone: " + milestone.get("id"),
                ResponseCode.CLIENT_ERROR.getResponseCode()
        );
      }


      String assessmentId = (String) assessmentDetailObj.get("identifier");
      if (StringUtils.isBlank(assessmentId)) {
        throw new ProjectCommonException(
                ResponseCode.invalidParameterValue.getErrorCode(),
                "Assessment identifier missing for milestone: " + milestone.get("id"),
                ResponseCode.CLIENT_ERROR.getResponseCode()
        );
      }
    ...
    logger.info(requestContext, "Learning Pathway validation successful for id: " + id);
  }
}
